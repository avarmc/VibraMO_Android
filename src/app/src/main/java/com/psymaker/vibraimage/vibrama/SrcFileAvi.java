 ...
 */
public class SrcFileAvi /* extends AndroidTestCase */ {
    private  static final String TAG = "SrcFileAvi";
    ...
    //   private   String INPUT_FILE = "v2.mp4";


    private ExtractMpegFramesWrapper mWrapper;

    public  SrcFile mFile;
    public  boolean mRealtime = true;

    public SrcFileAvi(SrcFile mFile) {
        this.mFile = mFile;
    }
    ...
    public Size getFileSize() {
        try {
            return extractMpegFrames1(false);
        } catch(Throwable th) {
    ...
    private  class ExtractMpegFramesWrapper implements Runnable {
        ...
        private SrcFileAvi mTest;
        private  Thread mThread;

        private ExtractMpegFramesWrapper(SrcFileAvi test) {
            mTest = test;
        }
        ...
        /** Entry point. */
        public  void runTest(SrcFileAvi obj) throws Throwable {
            ExtractMpegFramesWrapper wrapper = new ExtractMpegFramesWrapper(obj);
            mThread = new Thread(wrapper, "codec test");
            mThread.start();
            //     th.join();
    ...
    private void extractMpegFrames() throws IOException {
        mFile.mState = mFile.STATE_OPENED;
        while(mFile.mState == mFile.STATE_OPENED || mFile.mStarted) {
            extractMpegFrames1(true);
        }
    }
     ...
     */
    private Size extractMpegFrames1(boolean bExtract) throws IOException {
        ...
        try {
            ...
            File inputFile = new File(mFilePath); // new File(FILES_DIR, INPUT_FILE);   // must be an absolute path
        ...
        return new Size(saveWidth,saveHeight);
    }
    ...
    void doExtract(MediaExtractor extractor, int trackIndex, MediaCodec decoder,
        ...
        while (!outputDone && (mFile.mState == mFile.STATE_OPENED || mFile.mStarted)) {
            ...
            if (!outputDone) {
                ...
                if (decoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                    ...
                    if (doRender) {
                        ...
                        sampleTime = (t-startSample)/1000;
                        frameTime = (System.nanoTime() - startWhen)/1000000;

                        mFile.mTimeVideo = frameTime/1000.0f; //sampleTime/1000.0f;

                        if(mRealtime && decodeCount > 0 && frameTime < sampleTime) {
                            long dt = sampleTime-frameTime;
                            VideoBase.Sleep(dt);
                        }


                        // if (VERBOSE)
                        Log.d(TAG, "awaiting decode of frame " + decodeCount + " t="+sampleTime);
    ...
    private  void awaitNewImage(long nFrame, long nMax) {
        long t0 = System.nanoTime();
        long nsMax = nMax*1000000;
        while(System.nanoTime()-t0 < nsMax && nFrame == VideoBase.nFrame) {
            VideoBase.Sleep(1);
        }
    }
    /**
    ...
    private  class CodecOutputSurface
        ...
        private SrcFileAvi.STextureRender mTextureRender;
         ...
         */
        private void setup() {
            mTextureRender = new SrcFileAvi.STextureRender();
